...
#   session-summary-config install           # Show plugin install instructions
...
cmd_install() {
    echo "${BOLD}session-summary is installed via the plugin system.${RESET}"
    echo ""
    echo "The plugin system handles hook registration automatically."
    echo "Use ${CYAN}claude plugin${RESET} commands to manage installation:"
    echo ""
    echo "  ${CYAN}claude plugin marketplace add FlorianBruniaux/claude-code-plugins${RESET}"
    echo "  ${CYAN}claude plugin install session-summary@florian-claude-tools${RESET}"
    echo ""
    echo "To check status:  ${CYAN}claude plugin list${RESET}"
    echo "To uninstall:     ${CYAN}claude plugin uninstall session-summary${RESET}"
    echo ""
    echo "For manual installation (without plugin system), see:"
    echo "  ${DIM}https://github.com/FlorianBruniaux/claude-code-ultimate-guide/tree/main/examples/hooks${RESET}"
}
...
cmd_help() {
    ...
    echo "  session-summary-config ${CYAN}install${RESET}           Show plugin install instructions"
